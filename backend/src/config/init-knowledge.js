...
async function initKnowledge() {
  try {
    ...
    if (knowledgeCount === 0) {
      await db.Knowledge.bulkCreate(initialKnowledge);
      console.log('✅ 初始知识数据插入成功');
    } else {
      console.log('ℹ️ 知识表已有数据，跳过初始化');
    }
    
    const quizCount = await db.Quiz.count();
    if (quizCount === 0) {
      await db.Quiz.bulkCreate(initialQuizzes);
      console.log('✅ 初始测验数据插入成功');
    } else {
      console.log('ℹ️ 测验表已有数据，跳过初始化');
    }
