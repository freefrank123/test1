...
async function createTables() {
  try {
    ...
    if (response.ok) {
      ...
      console.log(`ℹ️ 创建表响应: ${response.status} - ${errorText.substring(0, 200)}`);
    }
    ...
    if (countErr) {
      console.error('❌ 查询表失败:', countErr.message);
      console.log('📋 请手动在 Supabase Dashboard > SQL Editor 中执行 supabase_migration.sql 文件');
      process.exit(1);
    }
    ...
    console.log('📋 请手动在 Supabase Dashboard > SQL Editor 中执行 supabase_migration.sql 文件来创建表');
    process.exit(1);
  }
}
