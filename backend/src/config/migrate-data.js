...
async function migrateData() {
  try {
    ...
    if (quizCountErr) {
      console.error('❌ quizzes 表不存在，请先在 Supabase Dashboard 中执行迁移脚本');
      process.exit(1);
    }
    ...
    if (quizCount === 0) {
      ...
      if (quizError) {
        ...
        console.log('✅ 测验题目数据迁移成功');
      }
    } else {
      console.log(`ℹ️ quizzes 表已有 ${quizCount} 条数据，跳过`);
    }
    ...
    if (knowledgeCountErr) {
      console.error('❌ knowledge_articles 表不存在');
    } else if (knowledgeCount === 0) {
      const { error: knowledgeError } = await supabase
        .from('knowledge_articles')
        .insert(knowledgeData.map(k => ({
          ...k,
          view_count: 0,
          created_at: new Date().toISOString(),
          updated_at: new Date().toISOString()
      ...
      if (knowledgeError) {
        ...
        console.log('✅ 知识文章数据迁移成功');
      }
    } else {
      console.log(`ℹ️ knowledge_articles 表已有 ${knowledgeCount} 条数据，跳过`);
    }

    console.log('\n🎉 数据迁移完成！');
    console.log('\n📊 数据统计：');
    console.log(`   - 测验题目: ${quizCount || quizData.length} 条`);
    console.log(`   - 知识文章: ${knowledgeCount || knowledgeData.length} 条`);
